...
CREATE TABLE Task (
  ...
  teamID INT FOREIGN KEY REFERENCES Team(id) ON DELETE CASCADE
);
...
INSERT INTO Project (id, name, startDate, endDate, Pdeadline)
VALUES
...
INSERT INTO Team (id, name, manager)
VALUES
...
INSERT INTO Employee (id, name, email, password, teamID)
VALUES
...
INSERT INTO Task (id, name, TKdeadline, projectID, teamID)
VALUES
...
SELECT projectID, Project.name , COUNT(*) as NumberofEmp
FROM
Employee JOIN Team ON teamID = id
JOIN Task ON teamID = id
JOIN Project ON id = Task.projectID
GROUP BY projectID, Project.name;
...
BEGIN
  BEGIN TRY
    ...
    SELECT @CurrentTeamID = teamID FROM Employee WHERE id = @Employeid;
